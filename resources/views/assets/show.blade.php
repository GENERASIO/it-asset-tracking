    ...
    <div class="py-8">
        <div class="max-w-4xl mx-auto sm:px-6 lg:px-8 space-y-4">
            ...
            @php
                ...
                $isStaff = auth()->user()->role === 'super_admin' || auth()->user()->role === 'it_staff';
            @endphp

            <!-- Breadcrumb + Actions -->
            <div class="flex items-center justify-between flex-wrap gap-3">
                <nav class="text-sm text-gray-500 dark:text-gray-400 flex items-center gap-1.5">
                    <a href="{{ $isStaff ? route('assets.index') : route('dashboard') }}"
                       class="hover:text-brand-600 dark:hover:text-brand-300 transition">Aset</a>
                    <span>/</span>
                    <span class="font-medium text-gray-700 dark:text-gray-200">{{ $asset->asset_code }}</span>
                </nav>

                <div class="flex flex-wrap gap-2">
                    @if ($isStaff)
                        <a href="{{ route('barcode.print', $asset) }}" target="_blank"
                           class="text-center bg-gray-100 dark:bg-gray-700 dark:text-gray-200 text-gray-700 px-3 py-1.5 rounded-lg text-sm transition-all duration-150 hover:scale-105 active:scale-95">Cetak Label</a>
                        <a href="{{ route('assets.edit', $asset) }}"
                           class="text-center bg-brand-500 hover:bg-brand-600 text-white px-3 py-1.5 rounded-lg text-sm transition-all duration-150 hover:scale-105 active:scale-95">Edit</a>
                    @endif
                    <a href="{{ $isStaff ? route('assets.index') : route('dashboard') }}"
                       class="text-center text-gray-500 dark:text-gray-400 px-3 py-1.5 text-sm">Kembali</a>
                </div>
            </div>

            <div class="bg-white dark:bg-gray-800 shadow rounded-lg p-6" x-data="{ tab: 'detail', photoIndex: 0 }">
                <!-- Hero Photo -->
                @if ($asset->photos->isNotEmpty())
                    <div class="relative rounded-lg overflow-hidden border border-gray-100 dark:border-gray-700 bg-gray-50 dark:bg-gray-900 mb-4" style="aspect-ratio: 16 / 9;">
                        @foreach ($asset->photos as $index => $photo)
                            <img src="{{ $photo->url }}" x-show="photoIndex === {{ $index }}"
                                 class="absolute inset-0 w-full h-full object-contain">
                        @endforeach

                        @if ($asset->photos->count() > 1)
                            <button type="button"
                                    @click="photoIndex = (photoIndex - 1 + {{ $asset->photos->count() }}) % {{ $asset->photos->count() }}"
                            ...
                            <button type="button"
                                    @click="photoIndex = (photoIndex + 1) % {{ $asset->photos->count() }}"
                            ...
                            <div class="absolute bottom-2 left-1/2 -translate-x-1/2 flex gap-1.5">
                                @foreach ($asset->photos as $index => $photo)
                                    <button type="button" @click="photoIndex = {{ $index }}"
                                            :class="photoIndex === {{ $index }} ? 'bg-white' : 'bg-white/50'"
                ...
                @else
                    <div class="rounded-lg border border-gray-100 dark:border-gray-700 bg-gray-50 dark:bg-gray-900 flex items-center justify-center text-gray-300 text-sm mb-4" style="aspect-ratio: 16 / 9;">
                        No Photo
                    </div>
                @endif
                ...
                <div class="border-b border-gray-200 dark:border-gray-700 mt-6">
                    <nav class="flex gap-6 -mb-px">
                        <button type="button" @click="tab = 'detail'"
                                :class="tab === 'detail' ? 'border-brand-500 text-brand-600 dark:text-brand-300' : 'border-transparent text-gray-500 dark:text-gray-400 hover:text-gray-700 dark:hover:text-gray-200 hover:border-gray-300'"
                                class="whitespace-nowrap border-b-2 py-3 px-1 text-sm font-medium transition">
                            Record Details
                        </button>
                        <button type="button" @click="tab = 'aktivitas'"
                                :class="tab === 'aktivitas' ? 'border-brand-500 text-brand-600 dark:text-brand-300' : 'border-transparent text-gray-500 dark:text-gray-400 hover:text-gray-700 dark:hover:text-gray-200 hover:border-gray-300'"
                                class="whitespace-nowrap border-b-2 py-3 px-1 text-sm font-medium transition">
                            Aktivitas
                        </button>
                    </nav>
                ...
                <!-- Tab: Record Details -->
                <div x-show="tab === 'detail'" class="pt-6 space-y-6">
                    ...
                    @if ($asset->last_seen_at)
                        <div class="border border-gray-100 dark:border-gray-700 rounded-lg p-4">
                            <div class="flex items-center justify-between mb-3">
                                <h3 class="font-semibold text-gray-700 dark:text-gray-300">Info dari Agent</h3>
                                <span class="text-xs px-2 py-1 rounded-full {{ $asset->last_seen_at->gt(now()->subDays(2)) ? 'bg-green-100 text-green-700' : 'bg-gray-100 text-gray-500' }}">
                                    Terakhir check-in {{ $asset->last_seen_at->diffForHumans() }}
                                </span>
                            </div>
                            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 text-sm">
                                <div>
                                    <span class="text-gray-500">Hostname</span>
                                    <p class="font-medium text-gray-900 dark:text-white">{{ $asset->hostname ?? '-' }}</p>
                                </div>
                                <div>
                                    <span class="text-gray-500">Sistem Operasi</span>
                                    <p class="font-medium text-gray-900 dark:text-white">{{ $asset->os_label ?? '-' }}</p>
                                </div>
                                <div>
                                    <span class="text-gray-500">RAM</span>
                                    <p class="font-medium text-gray-900 dark:text-white">{{ $asset->ram_gb ? $asset->ram_gb.' GB' : '-' }}</p>
                                </div>
                                <div>
                                    <span class="text-gray-500">Storage</span>
                                    <p class="font-medium text-gray-900 dark:text-white">{{ $asset->storage_gb ? $asset->storage_gb.' GB' : '-' }}</p>
                                </div>
                                <div class="sm:col-span-2">
                                    <span class="text-gray-500">MAC Address</span>
                                    <p class="font-medium text-gray-900 dark:text-white font-mono">{{ $asset->mac_address ?? '-' }}</p>
                                </div>
                            </div>
                        </div>
                    @endif
                        ...
                        <div class="border border-gray-100 dark:border-gray-700 rounded-lg p-4">
                            ...
                            @if ($asset->status === 'available')
                                <form action="{{ route('assets.checkout', $asset) }}" method="POST" class="space-y-3">
                                    @csrf
                                    <div>
                                        <label class="text-sm font-medium text-gray-700 dark:text-gray-300">Karyawan Peminjam</label>
                ...
                <!-- Tab: Aktivitas -->
                <div x-show="tab === 'aktivitas'" class="pt-6 space-y-6">
                    <div class="border border-gray-100 dark:border-gray-700 rounded-lg p-4">
                    ...
                    <div class="border border-gray-100 dark:border-gray-700 rounded-lg p-4">
                        ...
                        @if ($isStaff)
                        <form action="{{ route('maintenance-logs.store', $asset) }}" method="POST" class="space-y-3 mb-4 pb-4 border-b dark:border-gray-700">
                            @csrf
                            <div>
                                <label class="text-sm font-medium text-gray-700 dark:text-gray-300">Masalah/Keluhan</label>
                                <textarea name="issue" rows="2" required
                                          class="mt-1 block w-full border-gray-300 rounded-lg shadow-sm text-sm"></textarea>
                            </div>
                            <div>
                                <label class="text-sm font-medium text-gray-700 dark:text-gray-300">Teknisi (opsional)</label>
                                <input type="text" name="technician"
                                       class="mt-1 block w-full border-gray-300 rounded-lg shadow-sm text-sm">
                            </div>
                            <div>
                                <label class="text-sm font-medium text-gray-700 dark:text-gray-300">Tanggal Lapor</label>
                        ...
                        @forelse ($asset->maintenanceLogs as $log)
                            <div class="border-b dark:border-gray-700 py-2 text-sm">
                                ...
                                @if ($log->status !== 'done' && $isStaff)
                                    <form action="{{ route('maintenance-logs.update', $log) }}" method="POST"
                                          class="border-t dark:border-gray-700 pt-2 mt-2 text-xs space-y-1">
                                        @csrf
                                        @method('PUT')
